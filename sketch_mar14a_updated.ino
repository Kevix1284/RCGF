...
void setup() {
  ...
  pinMode(CH3, INPUT);
  //pinMode(CH7, INPUT);      //HLFB not implimented
  pinMode(CH6, OUTPUT);
  pinMode(CH8, OUTPUT);
  pinMode(CH4, OUTPUT);
  digitalWrite(4, HIGH);
  ...
  pinMode(CH9, INPUT);
  //pinMode(CH13, INPUT);      //HLFB not implimented
  pinMode(CH12, OUTPUT);
  pinMode(CH11, OUTPUT);
  pinMode(CH10, OUTPUT);
...
for(i == 0; i<2000; i++){
  ...
  ch3Value = readChannel(CH3, -100, 100, 0);      //Position of desired steering motor
  ch9Value = readChannel(CH9, -100, 100, 0);      //Position of desired brake motor
  if(ch9Value>0)
  ch9Value=0;
  ...
  pulsemotor(braking_change, braking_state, 10, 11, 12);
  pulsemotor(steering_change, steering_state, 4, 8, 6);
